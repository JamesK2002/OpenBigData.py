    ...
    for root, _, files in os.walk(folder):
        for f in files:
            ...
            try:
                ...
                print(f"⚠️ Could not change timestamp for {file_path}: {e}")
...
def replicate_until_60(original_folder, output_folder):
    ...
    for category in os.listdir(original_folder):
        ...
        if not csv_files:
            print(f"⚠️  No CSV files found in {category}")
            continue
        ...
        while get_folder_size(new_category_path) < target_size:
            ...
            print(f"  ✅ Cloned {num_to_clone} files... Current size: {get_folder_size(new_category_path)/(1024*1024):.2f} MB")
